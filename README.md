...
# Query for a : Find the titles of courses in the Comp. Sci. department that have 3 credits.
```sql
SELECT course_title
FROM Courses
...
## Query for _b: Find the IDs of all students who were taught by an instructor named Einstein; make sure there are no duplicates in the result.
```sql
SELECT s.student_id
FROM Students s
...
## Query for_c: Find the highest salary of any instructor.
```sql
...
## Query for_d: Find all instructors earning the highest salary (there may be more than
...
## Query for_e: Find the enrollment of each section that was offered in Fall 2017.
```sql
...
## Query for_f: Find the maximum enrollment, across all sections, in Fall 2017.
```sql
...
## Query for_g: Find the sections that had the maximum enrollment in Fall 2017.
```sql
...
## Query for _a: Find the total grade points earned by the student with ID '12345', across
...
```
## Query for _b: Find the grade point average (GPA) for the above student, that is, the total
...
## Query for _c: Find the ID and the grade-point average of each studen
```sql
SELECT 
    takes.student_id,
    SUM(course.credits * grade_points.points) / SUM(course.credits) AS GPA
...
```
## Query for _d: Now reconsider your answers to the earlier parts of this exercise under
...
```
## Query for problem 3.22: Rewrite the where clause where unique (select title from course)
...
```
## Query for problem 3.25 : Using the university schema, write an SQL query to find the names of those
## departments whose budget is higher than that of Philosophy. List them in alphabetic order.
```sql
...
```
## Query for problem 3.26 : Using the university schema, use SQL to do the following: For each student who
